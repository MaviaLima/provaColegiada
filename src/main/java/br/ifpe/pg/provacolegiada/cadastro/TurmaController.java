...
public class TurmaController {
	...
	private ModelAndView salvar(@Valid @ModelAttribute Turma turma, Errors errors, RedirectAttributes ra) {
		if (errors.hasErrors()) {
			...
			try {
				if(!cursoService.existeCursos()){
					ra.addFlashAttribute("mensagemErro", "Não foi possível salvar turma: " + "Não há cursos cadastrados!");
				} else if(turmaService.igual(turma)) {
					ra.addFlashAttribute("mensagemErro", "Não foi possível salvar turma: " + "Já existe uma turma com estas caracteristicas");
				}else {
